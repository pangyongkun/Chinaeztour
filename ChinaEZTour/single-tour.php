...
<section class="three-fourth">
	...
	<nav class="inner-nav">
		<ul>
            ...
            <li class="description active">
                <a href="#description" title="Itinerary">Itinerary</a>
            </li>
            <li class="availability">
                <a href="#availability" title="Price">Price Guide</a>
            </li>
            <li class="availability">
                <a href="#travel-tip" title="tip">Travel Tips</a>
            </li>
            <li class="availability">
                <a href="#service" title="service">Service</a>
            </li>
            <li class="things-to-do">
                <a href="#optional" title="option">Option</a>
            </li>
            <li class="map">
	...
	<!--description-->
	<section id="description"  class="tab-content <?php echo $first_display_tab == 'description' ? 'initial' : ''; ?>" style="display: block !important;">
		<article>
			...
			<?php $tour_activities=get_post_meta($tour_obj->get_id(),'tour_activity',ture);  ?>
			<?php
			$activity_day=1;
			foreach ($tour_activities as $tour_activity){
				...
				?>
				<div class="tour_day">
					<div class="tour_tittle"><b>Day <?php echo $activity_day++; ?> </b> <?php echo $tour_activity['day_title']; ?>(B,L,D)</div>
					<div class="tour_intro">

						<!--<div class="tour_meals">
					<b>Breakfast: </b><?php echo ($tour_activity['day_breakfast']==''||$tour_activity['day_breakfast']==null)?'No meals':$tour_activity['day_breakfast_detail'] ?>
					<b>Launch: </b><?php echo ($tour_activity['day_launch']==''||$tour_activity['day_launch']==null)?'No meals':$tour_activity['day_launch_detail'] ?>
					<b>Dinner: </b><?php echo ($tour_activity['day_dinner']==''||$tour_activity['day_dinner']==null)?'No meals':$tour_activity['day_dinner_detail'] ?>
				</div>-->
						<div class="tour_hotel"><b>Accommodation:</b> <?php echo $tour_activity['day_hotel']; ?></div>
						<div class="tour_meals"><b>Special meals:</b>Roasted Duck,Private kitchens</div>
						<div>
							<?php echo $tour_activity['day_detail']; ?>
						</div>
						<div class="tour_img">
							<?php $day_images=$tour_activity['day_image'];
							if(is_array($day_images)){
								foreach ($day_images as $day_image){
									$thumbnail = wp_get_attachment_image_src($day_image, full);
	...
	<section id="availability" class="tab-content <?php echo $first_display_tab == 'availability' ? 'initial' : ''; ?>">
		<article>
			...
			<form id="launch-tour-booking" action="#" method="POST">
				...
				if (count($schedule_entries) > 0) { ?>
					<div class="booking_form_controls_holder">
                        ...
                        <?php
                        /*group tour*/
                        $tour_type=$bookyourtravel_tour_helper->get_tour_type($tour_id);

                        if($tour_type->slug=='group-tour'){
                            ?>
                        ...
                        <?php }
                        /*private tour not display calender*/
                        ?>
			...
			if(get_the_terms($tour_id,'tour_type')[0]->slug=="private-tour"){
				?>
				<div class="text-wrap high_season"><h2>High Season</h2>
				<p><?php echo get_post_meta( $tour_id,'tour_high_season',true); ?></p>
				</div>
				<div class="text-wrap low_season"><h2>Low Season</h2>
				<p><?php echo get_post_meta( $tour_id,'tour_low_season',true); ?></p>
				</div>

				<div class="text-wrap high_season"><h2>Hotel</h2>
					<?php $tour_hotels=get_post_meta( $tour_id,'tour_hotel',true); ?>
					<p>
					<table class="table" width="100%" cellspacing="0">
						...
						<?php
						foreach ($tour_hotels as $tour_hotel){
						?>
						<tr>
							<td><?php echo $tour_hotel['tour_hotel_city']; ?></td>
							<td><?php echo $tour_hotel['tour_hotel_5_star']; ?></td>
							<td><?php echo $tour_hotel['tour_hotel_4_star']; ?></td>
							<td><?php echo $tour_hotel['tour_hotel_3_star']; ?></td>
						</tr>
						<?php } ?>
						</tbody>
					</table>
				...
				<div class="text-wrap high_season"><h2>Hotel price</h2>
					<?php $tour_hotel_prices=get_post_meta( $tour_id,'tour_hotel_price',true); ?>
					<p>
					<table class="table" width="100%" cellspacing="0">
						<tbody>
						<tr align="center">
							<th>Group size(person)</th>
							<th>5 Star ($)</th>
							<th>4 Star ($)</th>
							<th>3 Star ($)</th>
						</tr>
						<?php
						foreach ($tour_hotel_prices as $tour_hotel_price){
							?>
							<tr>
								<td><?php echo $tour_hotel_price['tour_group_size']; ?></td>
								<td><?php echo $tour_hotel_price['tour_hotel_5_star']; ?></td>
								<td><?php echo $tour_hotel_price['tour_hotel_4_star']; ?></td>
								<td><?php echo $tour_hotel_price['tour_hotel_3_star']; ?></td>
							</tr>
						<?php } ?>
						</tbody>
					</table>
			...
			<?php $tour_inclusion=get_post_meta( $tour_id,'tour_inclusion',true);  ?>
				<div class="text-wrap whats_include"><h2>What's Include</h2>
				<p><?php echo $tour_inclusion['tour_inclusion_include']; ?></p>
				</div>

				<div class="text-wrap whats_exclude"><h2>What's Exclude</h2>
				<p><?php echo $tour_inclusion['tour_inclusion_exclude']; ?></p>
				</div>
    ...
    <!--travel tips-->
    <section id="travel-tip" class="tab-content">
        <article>
            <?php echo get_post_meta($tour_obj->get_id(),'tour_travel_tip',ture); ?>
        </article>
    </section>
    <!--//travel tips-->

    <!--travel tour_services-->
    <section id="service" class="tab-content">
        <article>
            <?php echo get_post_meta($tour_obj->get_id(), 'tour_services', ture); ?>
        </article>
    </section>
    <!--//travel tour_services-->

    <!--travel optional-->
    <section id="optional" class="tab-content">
        <article>
            <?php echo get_post_meta($tour_obj->get_id(), 'tour_optional_activities', ture); ?>
        </article>
    </section>
    <!--//travel optional-->
