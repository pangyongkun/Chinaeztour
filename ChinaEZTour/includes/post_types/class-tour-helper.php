...
/*				array(
				...
				array( // Post ID select box
					...
					'type'	=> 'location_checkboxes', // type of field
					'post_type' => array('location') // post types to display, options are prefixed with their post type
				),
                /*新增字段*/
                array( // Post ID select box
                    'label'	=> esc_html__('Start City', 'bookyourtravel'), // <label>
                    'desc'	=> '', // description
                    'id'	=> 'tour_start_city', // field id and name
                    'type'	=> 'location_checkboxes', // type of field
                    'post_type' => array('location') // post types to display, options are prefixed with their post type
                ),
...
/*				array(
	...
	function tours_search_groupby( $groupby, &$wp_query ) {
		...
		if (!is_admin()) {
			if ( isset($wp_query->query_vars['post_type']) && $wp_query->query_vars['post_type'] == 'tour' ) {
                ...
                /*if(isset($wp_query->query_vars['duration_day'])){
                    $groupby .= ' AND ( 1!=1 ';
                    $groupby.=' AND ez_byt_tour_schedule.duration_days > 8 ';
                    $groupby .= ") ";
                }*/
    ...
    /*search tours*/
	function list_tours($paged = 0, $per_page = -1, $orderby = '', $order = '', $location_id = array(), $tour_types_array = array(), $tour_tags_array = array(), $search_args = array(), $featured_only = false, $author_id = null, $include_private = false, $count_only = false ) {
        ...
        /*locations*/
        if(isset($search_args['locations'])&& count($search_args['locations'])>1){
            $locations=$search_args['locations'];
        }

        /*start cities*/
        if(isset($search_args['start_city']) && count($search_args['start_city'])>1){
            $start_cities=$search_args['start_city'];
        }

        /*duration day*/
        if(isset($search_args['duration_day'])&& $search_args['duration_day']!=null&&$search_args['duration_day']!=''){
            $duration_day=$search_args['duration_day'];
        }
        ...
        if (count($start_cities) > 0) {
            //$location_ids_str = implode(',', $location_ids);
            $meta_query_array = array(
                'relation' => 'OR'
            );

            foreach ($start_cities as $location_id) {
                $meta_query_array[] = array(
                    'key' => 'tour_start_city',
                    'value' => serialize(strval($location_id)),
                    'compare' => 'LIKE'
        ...
        if($duration_day!=null&&$duration_day!=''){
            $args['duration_day']=$duration_day;
        }
    ...
    /*获取tour_type*/
    function get_tour_type($tour_id) {

        return get_the_terms($tour_id,'tour_type')[0];
    }
